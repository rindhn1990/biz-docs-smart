...
import { ScanMock } from "@/components/ScanMock";
import { formatFileSize } from "@/lib/format";
import { EMPLOYEE_DOC_TYPES, type EmployeeDocType } from "@/lib/hr";

export const Route = createFileRoute("/_app/nhan-su/$documentId")({
  head: () => ({
    meta: [
      { title: "Kiểm tra hồ sơ nhân sự — OfficeFlow" },
      {
        name: "description",
        content:
          "Đối chiếu dữ liệu trích xuất từ quyết định bổ nhiệm, căn cước công dân hoặc bằng cấp với bản scan gốc trước khi xác nhận.",
      },
      { property: "og:title", content: "Kiểm tra hồ sơ nhân sự — OfficeFlow" },
      {
        property: "og:description",
        content: "Màn hình đối chiếu dữ liệu nhân sự với tài liệu gốc trước khi lưu.",
      },
    ],
  }),
...
function EmployeeReviewPage() {
  ...
  const doc = useQuery({
    queryKey: ["employee_document", documentId],
    queryFn: async () => {
  ...
  const fields = useQuery({
    queryKey: ["employee_document_fields", documentId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("employee_document_fields")
        .select("*")
  ...
  const approve = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Đã xác nhận và lưu hồ sơ nhân sự");
  ...
  const lowConfidence = rows.filter((f) => Math.round(Number(f.confidence) * 100) < 85).length;
  ...
  if (!doc.data) {
    return (
      <div className="py-16 text-center">
        ...
        <button
          onClick={() => void navigate({ to: "/nhan-su" })}
  ...
  const type = EMPLOYEE_DOC_TYPES[doc.data.doc_type as EmployeeDocType];
      ...
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {type?.label ?? doc.data.doc_type} · {doc.data.page_count ?? "—"} trang ·{" "}
            {formatFileSize(doc.data.file_size)}
            {doc.data.employees?.full_name ? ` · ${doc.data.employees.full_name}` : ""}
          </p>
        </div>
...
function FieldRowEditor({
  ...
  const save = async (next: string) => {
    ...
    void queryClient.invalidateQueries({
      queryKey: ["employee_document_fields", field.document_id],
    });
  };
